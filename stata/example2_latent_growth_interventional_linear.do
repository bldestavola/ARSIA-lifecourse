...
set more off
set seed 210404
log using example2_latent_growth_interventional_linear_200421.log,replace
...
mixed l_bmi c_age std_bw c1 c2 c3 c4 matBMI age_bw age_matBMI ||newid: c_age if original==1,  cov(unstr) nolog stddev
...
#delimit ;
	gen alpha=_b[_cons] +_b[std_bw]*std_bw  
...
#delimit ;
	gen alpha_`v'`v'=_b[_cons] +_b[std_bw]*`v'
              +_b[c1]*c1+_b[c2]*c2+_b[c3]*c3+_b[c4]*c4+_b[matBMI]*matBMI
			  +(intercept+i);
#delimit cr
}
...
*************M2************************************
*conditional model
reg beta alpha std_bw  c1 c2 c3 c4 matBMI
cap drop UB
qui gen UB=rnormal()
...
*marginal model
reg beta std_bw c1 c2 c3 c4 matBMI
cap drop UB
qui gen UB=rnormal()

foreach v of numlist 0/1{		  
#delimit ;
	gen beta_`v'`v'_marg=_b[_cons] +_b[std_bw]*`v' 
					+_b[c1]*c1+_b[c2]*c2+_b[c3]*c3+_b[c4]*c4 + _b[matBMI]*matBMI	
					  +e(rmse)*UB;
#delimit cr
}
...
reg	y std_bw alpha beta c1 c2 c3 c4 matBMI if original==1
cap drop UY
qui gen UY=rnormal()
